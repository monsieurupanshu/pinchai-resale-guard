...
def gen_loyal_bulk_buyers(n: int) -> list:
    ...
    customers = []
    for _ in range(n):
        # established accounts — bulk trust requires history
...
def gen_solo_resellers(n: int) -> list:
    """Single account, newer, concentrated bulk buys timed to discount
    windows."""
    customers = []
    for _ in range(n):
...
def gen_ring_resellers(n_rings: int, accounts_per_ring=(2, 6)) -> list:
    ...
    for _ in range(n_rings):
        ...
        shared_address = _new_id("ADDR")
        # rings vary in HOW much they share — some share everything (sloppy),
        # some share only address (careful), which makes detection realistic
        ...
        for _ in range(k):
            created = burst_start + timedelta(minutes=random.uniform(0, 240))
...
def gen_orders_loyal_bulk(customers, windows) -> list:
    ...
    for c in customers:
        ...
        for _ in range(max(1, n_orders)):
            sku = random.choice(SKU_CATALOG)  # diverse SKU choice, not concentrated
            qty = int(np.random.choice([3, 4, 5, 6, 8], p=[0.35, 0.25, 0.2, 0.12, 0.08]))
            # buys spread across the whole timeline, NOT concentrated on discount windows
            ts = _rand_ts(max(SIM_START, c.account_created), SIM_END)
            discount_pct = 0.0
            price = sku["retail_price"]
            for w in windows:
...
def gen_orders_solo_reseller(customers, windows) -> list:
    orders = []
    for c in customers:
        # concentrate on 1-2 SKUs, buy heavily and specifically during
        # discount windows
    ...
    for ring_id, members in by_ring.items():
        ...
        for w in chosen_windows:
            ...
            if not eligible:
                continue
            sku = random.choice(eligible)
            # each member places ONE order under the per-account limit,
            # but collectively the ring buys a large quantity
            for c in members:
                qty = int(np.random.choice([3, 4, 5], p=[0.5, 0.3, 0.2]))  # under typical per-account cap
